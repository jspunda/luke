...
def get_entity_spans(s, p):
    matches = p.finditer(s)
    spans = [m.span(1) for m in matches]
    if len(spans) != 2:
        raise RuntimeError("Entity spans could not be extracted from: {}".format(s))
    else:
        return spans


class DatasetProcessor(object, metaclass=abc.ABCMeta):
...
class TacredDatasetProcessor(DatasetProcessor):
    def _create_examples(self, data_dir, set_type):
...
class KBP37DatasetProcessor(DatasetProcessor):
    ENTITY_RE = re.compile(r"<e[1-2]> ([^<]+) </e[1-2]>")  # Used to extract entity spans from raw data text files

    def _create_examples(self, data_dir, set_type):
        dataset_filename = os.path.join(data_dir, set_type + ".txt")
        data = []
        with open(dataset_filename, "r") as f:
            i = 0
            for line in f:
                if len(line) > 1:  # Data contains a lot of empty lines
                    _, text = line.split("\t")
                    text = text.replace('"', "").strip()

                    # Label is always one line under the text
                    label = f.readline().strip().replace("(e1,e2)", "").replace("(e2,e1)", "")
                    try:
                        s1, s2 = get_entity_spans(text, self.ENTITY_RE)
                        text = text.replace("<e1> ", "").replace(" </e1>", "").replace("<e2> ", "").replace(
                            " </e2>", "")
                        # To adjust for removed entity markers (<e1>, </e1>, <e2>, </e2>)
                        updated_span1 = (s1[0] - 5, s1[1] - 5)
                        updated_span2 = (s2[0] - 16, s2[1] - 16)
                        spans = [updated_span1, updated_span2]
                    except RuntimeError as e:
                        logger.warning(str(e))
                        continue
                    data_point = InputExample(
                        "%s-%s" % (set_type, i),
                        text,
                        spans[0],
                        spans[1],
                        None,  # There is no entity type present in KBP37
                        None,  # There is no entity type present in KBP37
                        label
                    )
...
class SemevalDatasetProcessor(DatasetProcessor):
