...
from .utils import HEAD_TOKEN, TAIL_TOKEN, convert_examples_to_features, DatasetProcessor, KBP37DatasetProcessor, \
    TacredDatasetProcessor, SemevalDatasetProcessor
...
@cli.command()
@click.option("--dataset", default="tacred", type=click.Choice(["tacred", "kbp37", "semeval"]))
...
def load_examples(args, fold="train"):
    ...
    if args.dataset == "kbp37":
        processor = KBP37DatasetProcessor()
    elif args.dataset == "semeval":
        processor = SemevalDatasetProcessor()
    else:
        processor = TacredDatasetProcessor()
